...
import type {ContentTable, TranscriptItem, UserVideosItem, Video} from "@shared/types";
    ...
    getVideo(req: UserRequest, res: Response) {
        ...
        validateUserAndVideo(req, res, videoId).then((result) => {
            ...
            user.populate({
                ...
                const userVideo = populatedUser.userVideos.find((userVideo: UserVideosItem) => (userVideo.videoId as Video).id.toString() === videoId);
